...
class Model(base_layer.Layer, version_utils.ModelVersionSelector):
  ...
  '''
  CS 635 ORIGINAL FUNC COMMENTED OUT FOR REFERENCE
  '''
  ...
  '''
  CS635 Assignment Refactoring:
    Refactored init function. Started out long and complicated,
    split into multiple helper functions to help with simplification
    and readability
  '''
  @trackable.no_automatic_dependency_tracking
  def __init__(self, *args, **kwargs):
      ...
      # Check if the model is a subclassed functional model
      if self._is_subclassed_functional_model(args, kwargs):
          self._handle_functional_model_initialization(args, kwargs)
          return

      # Validate keyword args, init model w/ helper methods
      self._validate_kwargs(kwargs)
      self._initialize_attributes(kwargs)
      self._initialize_compile_settings()

  @trackable.no_automatic_dependency_tracking
  def _is_subclassed_functional_model(self, args, kwargs):
      """
      Helper function to check if the model is a subclassed functional model
      """
      from tensorflow.python.keras.engine import functional  # pylint: disable=g-import-not-at-top

      # Check if the model is initialized with functional model params
      return (is_functional_model_init_params(args, kwargs) and
              not isinstance(self, functional.Functional))

  @trackable.no_automatic_dependency_tracking
  def _handle_functional_model_initialization(self, args, kwargs):
      """
      Handle initialization when the model is a subclassed functional model.
      Initialize functional model, handle any multiple inheritance
      """
      from tensorflow.python.keras.engine import functional  # pylint: disable=g-import-not-at-top
      supported_kwargs = ['inputs', 'outputs', 'name', 'trainable', 'skip_init']
      model_kwargs = {k: kwargs[k] for k in kwargs if k in supported_kwargs}
      ...
      # Inject functional model class and init functional model
      inject_functional_model_class(self.__class__)
      functional.Functional.__init__(self, *args, **model_kwargs)

      # Init any classes in multiple inheritance chains
      self._init_multiple_inheritance_classes(args, other_kwargs)

  @trackable.no_automatic_dependency_tracking
  def _validate_kwargs(self, kwargs):
      """
      Validates provided keyword args to ensure they are supported
      Raises error if unsupported keyword args are found
      """
      valid_kwargs = {'trainable', 'dtype', 'dynamic', 'name', 'autocast', 'inputs', 'outputs'}
      generic_utils.validate_kwargs(kwargs, valid_kwargs)

  @trackable.no_automatic_dependency_tracking
  def _initialize_attributes(self, kwargs):
      """
      Initializes attributes related to model config and settings
      """
      super(Model, self).__init__(**kwargs)
      self._is_graph_network = False
      self.inputs = None
      ...
      # Init the distribution strategy (if exists)
      if distribute_lib.has_strategy():
          self._distribution_strategy = distribute_lib.get_strategy()
      else:
          self._distribution_strategy = None
  ...
  @trackable.no_automatic_dependency_tracking
  def _initialize_compile_settings(self):
      """
      Initializes settings related to model compilation to help setup for training
      """
      self.compiled_loss = None
      self.compiled_metrics = None
      self._compute_output_and_mask_jointly = False
  ...
  @trackable.no_automatic_dependency_tracking
  def _init_multiple_inheritance_classes(self, args, other_kwargs):
      """
      Initialize classes in inheritance chain after functional class
      """
      from tensorflow.python.keras.engine import functional  # pylint: disable=g-import-not-at-top
      ...
      # Iterate through base classes and find those after functional
      for clz in self.__class__.__bases__:
          if issubclass(clz, functional.Functional):
              found_functional_class = True
              continue
      ...
      # Initialize classes if found
      if clz_to_init:
          for clz in clz_to_init:
              clz.__init__(self, *args, **other_kwargs)
      elif other_kwargs:
          # Raise error if unsupported keyword args are provided
          raise TypeError('The following keyword arguments aren\'t supported: {}'.format(other_kwargs))
